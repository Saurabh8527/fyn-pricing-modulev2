...
class CalculateTotalPriceTestCase(TestCase):
    def setUp(self):
        ...
        self.distance_base_price = DistanceBasePrice.objects.create(
            config=self.config, day_of_week='Wed', base_distance_km=10, base_price=50, additional_distance_price_per_km=2
        )
        ...
        self.waiting_charges = WaitingCharges.objects.create(
            config=self.config, initial_wait_min=10, price_per_min=1.5
        )
    ...
    def test_invalid_date_format(self):
        ...
        data = {
            ...
            'trip_time': 45,
            'waiting_time': 20,
            'trip_date': '2023/08/02',   # Invalid date format
        }
        response = self.client.get(url, data)
