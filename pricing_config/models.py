...
class DistanceBasePrice(models.Model):
    ...
    def clean(self):
        if self.base_distance_km <= 0:
            raise ValidationError("Base distance should be greater than 0.")
        if self.base_price <= 0:
            raise ValidationError("Base price should be greater than 0.")
...
class TimeMultiplierFactor(models.Model):
    ...
    def clean(self):
        if self.after_duration_mins < 0:
            raise ValidationError("After duration should be non-negative.")
        if self.till_duration_mins and self.till_duration_mins < self.after_duration_mins:
            raise ValidationError(
                "Till duration should be greater than or equal to after duration.")
        if self.multiplier_factor <= 0:
            raise ValidationError(
                "Multiplier factor should be greater than 0.")
...
class WaitingCharges(models.Model):
    ...
    def clean(self):
        if self.initial_wait_min < 0:
            raise ValidationError("Initial wait time should be non-negative.")
        if self.price_per_min < 0:
            raise ValidationError("Price per min should be greater than 0.")
